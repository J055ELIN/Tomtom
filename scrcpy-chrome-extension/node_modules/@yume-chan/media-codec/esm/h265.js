 ...
 */
export function parseNaluHeader(nalu) {
 ...
 */
export function parseVideoParameterSet(nalu) {
    const reader = new NaluSodbBitReader(nalu);
 ...
 */
export function parseScalingListData(reader) {
    const scaling_list = [];
 ...
 */
export function parseShortTermReferencePictureSet(reader, stRpsIdx, num_short_term_ref_pic_sets, sets) {
 ...
 */
export function parseVuiParameters(reader, sps_max_sub_layers_minus1) {
 ...
 */
export function parseHrdParameters(reader, commonInfPresentFlag, maxNumSubLayersMinus1) {
 ...
 */
export function parseSubLayerHrdParameters(reader, subLayerId, CpbCnt) {
...
export function parseSpsMultilayerExtension(reader) {
    const inter_view_mv_vert_constraint_flag = !!reader.next();
    return {
        inter_view_mv_vert_constraint_flag,
    };
}
export function parseSps3dExtension(reader) {
...
export function parseConfiguration(data) {
    ...
    const { chroma_format_idc, pic_width_in_luma_samples: encodedWidth, pic_height_in_luma_samples: encodedHeight, conf_win_left_offset: cropLeft = 0, conf_win_right_offset: cropRight = 0, conf_win_top_offset: cropTop = 0, conf_win_bottom_offset: cropBottom = 0, } = parseSequenceParameterSet(sequenceParameterSet.rbsp);
