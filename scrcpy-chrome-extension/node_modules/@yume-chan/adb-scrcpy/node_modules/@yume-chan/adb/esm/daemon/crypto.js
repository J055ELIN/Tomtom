...
export function rsaParsePrivateKey(key) {
    if (key.length < RsaPrivateKeyDOffset + RsaPrivateKeyDLength) {
        throw new Error("RSA private key is too short. Expecting a PKCS#8 formatted RSA private key with modulus length 2048 bits and public exponent 65537.");
    }
...
export function rsaSign(privateKey, data) {
    if (data.length !== SHA1_DIGEST_LENGTH) {
        throw new Error(`rsaSign expects ${SHA1_DIGEST_LENGTH} bytes (SHA-1 digest length) of data but got ${data.length} bytes`);
    }
